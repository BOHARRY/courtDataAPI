...
async function enhanceLawQuery(userQuery, context = '') {
    try {
        console.log(`[LawSearch] 使用 GPT-4.1-mini 優化法條查詢: "${userQuery}"`);
...
## 第一步：識別核心法律爭議
請先判斷此問題的主要法律爭議類型（可能包含多種）：
- **刑事違法行為**：是否涉及刑法規定的犯罪行為
- **侵權行為**：是否造成他人權利受損
- **契約違反**：是否違反雙方約定
- **行政違規**：是否違反行政法規
- **權利保護**：涉及何種權利需要保護
...
## 第三步：具體排序範例
**範例：「房東未經同意換鎖」問題**
1. 刑法第306條（侵入住宅罪）→ 優先級1：可能構成犯罪
2. 刑法第304條（強制罪）→ 優先級1：妨害自由
3. 民法第184條（侵權行為）→ 優先級2：損害賠償
4. 租賃條例第8條（不得妨礙承租人居住使用）→ 優先級2：特別法保護
5. 民法第423條（出租人修繕義務）→ 優先級3：行為規範
6. 民法第438條（承租人返還義務）→ 優先級3：相關義務
7. 其他租賃相關條文 → 優先級4：補充性條文

## 請提供：
1. **10個最相關的具體台灣法條**（嚴格依照上述優先級排序）：
   - 請先列出所有優先級1的條文
   - 再列出優先級2的條文
   - 依此類推
   - 使用標準格式：
